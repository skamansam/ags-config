/**
* This file stolen from https://github.com/ikatyang/emoji-cheat-sheet/tree/master
and modified to generate a json file with the emojis
instead of a markdown file
*/
import emojiMap from "markdown-it-emoji/lib/data/full.mjs";
import emoticonMap from "markdown-it-emoji/lib/data/shortcuts.mjs";
    ...
    Object.entries(
      ...
      url.includes('/unicode/')
        ? url.split('/').at(-1)
            ...
            .join('')
        : [url.split('/').at(-1).split('.png')[0]], // github's custom emoji
    ]),
  )
}
...
export async function getCategorizeGithubEmojiIds() {
  ...
  const categorizedEmojiMap = Object.entries(categorizedEmojiIds).reduce((acc, [category, subcategories]) => {
    console.log(category);
    acc[category] = Object.entries(subcategories).reduce((acc2, [subcategory, emojis]) => {

      acc2[subcategory] = emojis.flat().reduce((acc3, emojiName) => {
        acc3[emojiName] = emojiMap[emojiName] || emoticonMap[emojiName] || null;
        return acc3;
      }, {});
      return acc2;
    }, {})
...
console.log(JSON.stringify(categorizedGithubEmojiIds, null, 2))
